-- 创建时间:2022-02-10
local cur_path = "Game.game_MiniGame.Lua."
ext_require(cur_path .. "MiniGameModel")
ext_require(cur_path .. "MiniGameGamePanel")

MiniGameLogic = {}
local L = MiniGameLogic
L.panelNameMap = {
    game = "game",
    hall = "hall"
}
...
function L.Init(parm)
    this = L
    dump(parm, "<color=red>MiniGameLogic Init parm</color>")
    --初始化model
...
function L.Exit()
    if this then
        print("<color=green>Exit  MiniGameLogic</color>")
        this = nil
        if cur_panel then
...
function L.change_panel(panelName)
    ...
    if not cur_panel then
        ...
        elseif panelName == L.panelNameMap.game then
            cur_panel = {name = panelName, instance = MiniGameGamePanel.Create()}
        end
    end
end

-- function L.quit_game(call, quit_msg_call)
--     Network.SendRequest("xxx发送退出协议", nil, "", function (data)
