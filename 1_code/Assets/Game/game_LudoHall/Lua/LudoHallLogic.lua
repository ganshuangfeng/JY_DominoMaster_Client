-- 创建时间:2021-12-02
local cur_path = "Game.game_LudoHall.Lua."
ext_require(cur_path .. "LudoHallModel")
ext_require(cur_path .. "LudoHallGamePanel")

LudoHallLogic = {}
local L = LudoHallLogic
L.panelNameMap = {
    game = "game",
    hall = "hall"
}
...
function L.Init(parm)
    this = L
    dump(parm, "<color=red>LudoHallLogic Init parm</color>")
    --初始化model
    local model = LudoHallModel.Init()
    MakeLister()
    AddMsgListener(lister)
...
function L.Exit()
    if this then
        print("<color=green>Exit  LudoHallLogic</color>")
        this = nil
        if cur_panel then
...
function L.change_panel(panelName)
    ...
    if not cur_panel then
        ...
        elseif panelName == L.panelNameMap.game then
            cur_panel = {name = panelName, instance = LudoHallGamePanel.Create()}
        end
    end
end
