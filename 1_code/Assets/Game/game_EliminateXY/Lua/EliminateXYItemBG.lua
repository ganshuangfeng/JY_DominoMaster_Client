...
EliminateXYItemBG = basefunc.class()
local M = EliminateXYItemBG

math.randomseed(tostring(os.time()):reverse():sub(1, 6))
M.random = math.random(1,2)

M.name = "EliminateXYItemBG"
...
function M:ctor(data)
    local parent = EliminateXYObjManager.GetBGContent()
    if not parent then return end
    self.data = data
    self.ui = {}
	self.ui.gameObject = GameObject.Instantiate(EliminateXYObjManager.item_obj.EliminateXYItemBG,parent)
    self.ui.transform = self.ui.gameObject.transform
    self.ui.transform.localPosition = eliminate_xy_algorithm.get_bg_pos_by_index(self.data.x,self.data.y)
...
function M:Refresh(data)
    if true then return end
    if data.id == EliminateXYModel.eliminate_enum.swk then
