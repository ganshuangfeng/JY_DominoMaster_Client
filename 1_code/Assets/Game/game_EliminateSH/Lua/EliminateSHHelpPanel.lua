...
function C:InitUI()
	...
	for xc_count = 3,8 do
		...
		for xc_id = 1,5 do
			...
			ui_table["id" .. xc_id .. "_txt"].text = eliminate_sh_algorithm.get_rate(xc_id,xc_count) .. "x"
		end
	end
	...
	for xc_count = 3,8 do
		...
		ui_table["id2_txt"].text = eliminate_sh_algorithm.get_rate(6,xc_count) .. "x"
		local hero_count = eliminate_sh_algorithm.get_hero_count(xc_count)
		if hero_count then
			ui_table["id1_txt"].text = hero_count .. " pendekar atau"
		end
	end
end
