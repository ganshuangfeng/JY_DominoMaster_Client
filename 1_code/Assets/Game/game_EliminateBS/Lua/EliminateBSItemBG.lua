...
EliminateBSItemBG = basefunc.class()
local M = EliminateBSItemBG

math.randomseed(tostring(os.time()):reverse():sub(1, 6))
M.random = math.random(1,2)

M.name = "EliminateBSItemBG"
...
function M:ctor(data)
    local parent = EliminateBSObjManager.GetBGContent()
    if not parent then return end
    self.data = data
    self.ui = {}
	self.ui.gameObject = GameObject.Instantiate(EliminateBSObjManager.item_obj.EliminateBSItemBG,parent)
    self.ui.transform = self.ui.gameObject.transform
    self.ui.transform.localPosition = eliminate_bs_algorithm.get_bg_pos_by_index(self.data.x,self.data.y)
