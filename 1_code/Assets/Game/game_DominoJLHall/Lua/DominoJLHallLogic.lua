-- 创建时间:2021-12-02
local cur_path = "Game.game_DominoJLHall.Lua."
ext_require(cur_path .. "DominoJLHallModel")
ext_require(cur_path .. "DominoJLHallGamePanel")

DominoJLHallLogic = {}
local L = DominoJLHallLogic
L.panelNameMap = {
    game = "game",
    hall = "hall"
}
...
function L.Init(parm)
    this = L
    this.parm = parm
    dump(parm, "<color=red>DominoJLHallLogic Init parm</color>")
    --初始化model
    local model = DominoJLHallModel.Init()
    MakeLister()
    AddMsgListener(lister)
...
function L.Exit()
    if this then
        print("<color=green>Exit  DominoJLHallLogic</color>")
        this = nil
        if cur_panel then
...
function L.change_panel(panelName)
    ...
    if not cur_panel then
        ...
        elseif panelName == L.panelNameMap.game then
            if not this.parm then
                local type = PlayerPrefs.GetInt(MainModel.UserInfo.user_id.."domino_last_game_type",1)
                this.parm = {domino_type = type}
            else
                PlayerPrefs.SetInt(MainModel.UserInfo.user_id.."domino_last_game_type",this.parm.domino_type)
            end 
            cur_panel = {name = panelName, instance = DominoJLHallGamePanel.Create(this.parm)}
        end
    end
end
--改变模式
function L.change_pattern(type)
    this.parm = {domino_type = type}
    PlayerPrefs.SetInt(MainModel.UserInfo.user_id.."domino_last_game_type",this.parm.domino_type)
    cur_panel.instance:MyExit()
    cur_panel = {name = "hall", instance = DominoJLHallGamePanel.Create(this.parm)}
end
