...
function C:MyExit()
	...
	Event.Brocast("hall_panel_exit_msg")
...
function C:AddListenerGameObject()
	...
	self.slots_btn.onClick:AddListener(function ()
		...
		GameManager.CommonGotoScence({gotoui = "game_SlotsHall",})

		-- if MainModel.UserInfo.jing_bi < 10000000 then
	...
	self.slots_lion_btn.onClick:AddListener(function ()
		ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
		-- Event.Brocast("bsds_send_power",{key = "btn_14"})
		...
		-- MainModel.SetLastGameID(-1)
		print("<color=green>舞狮进入？？？？？？</color>")
		GameManager.CommonGotoScence({gotoui = "game_SlotsLion",}, function()
			-- Event.Brocast("slot_wushi_enter_game_response","slot_wushi_enter_game_response",{result = 0})
			Network.SendRequest("slot_wushi_enter_game",nil,nil,function (data)
				dump(data,"<color=yellow>slot_wushi_enter_game</color>")
				if data.result == 0 then
	...
	self.rp_dh_btn.onClick:AddListener(function()
		MainModel.OpenDH()
	end)
end
...
function C:RemoveListenerGameObject()
	...
	self.head_info_btn.onClick:RemoveAllListeners()
	self.more_btn.onClick:RemoveAllListeners()
	self.lhd_btn.onClick:RemoveAllListeners()
	self.slots_btn.onClick:RemoveAllListeners()
	self.start_btn.onClick:RemoveAllListeners()
	self.qiuqiu_btn.onClick:RemoveAllListeners()
	self.bm_bet_btn.onClick:RemoveAllListeners()
	self.ludo4_btn.onClick:RemoveAllListeners()
	self.ludo2_btn.onClick:RemoveAllListeners()
	self.bm_btn.onClick:RemoveAllListeners()
	self.rp_btn.onClick:RemoveAllListeners()
	self.add_money_btn.onClick:RemoveAllListeners()
	self.rp_dh_btn.onClick:RemoveAllListeners()
end

function C:InitUI()
	local btn_map = {}
	btn_map["down"] = {self.d_node1,self.d_node2,self.d_node3,self.d_node4,self.d_node5,self.d_node6,self.d_node7}
	btn_map["top"] = {self.t_node1,self.t_node2,self.t_node3,self.t_node4,self.t_node5,self.t_node6}
	btn_map["right"] = {self.r_node1,self.r_node2,self.r_node3,self.r_node4,self.r_node5}
    ...
    Event.Brocast("game_run_guide_msg",{name = "hall", call=function ()
        GameManager.GotoUI({gotoui = "sys_login_display",goto_scene_parm="panel"})
    end})

	Event.Brocast("hall_panel_init_msg", self)
end
function C:SetOnOff()
end
...
function C:DZ()
	...
	local txt1 = CustomUITool.CreateText({name="Text1", size={x=200,y=100},pos={x=-460,y=100,z=0},text="财神"}, obj1.transform)
	...
	local txt1 = CustomUITool.CreateText({name="Text1", size={x=200,y=100},pos={x=-460,y=0,z=0},text="超级"}, obj1.transform)
	EventTriggerListener.Get(txt1.gameObject).onClick = function ()
		GameManager.CommonGotoScence({gotoui = "game_EliminateCJ"})
	end
...
function C:onStartBtn()
	...
	if MainModel.UserInfo.jing_bi >= (info.limit_min or 0) then
		...
		elseif info.game_name == "game_LZHD" then
			ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
			Event.Brocast("bsds_send_power",{key = "btn_23"})
			GameManager.CommonGotoScence({gotoui = "game_LZHD",}, function()
