...
local config = require "Game/game_SlotsHall/Lua/slots_hall_config"

SlotsHallModel = {}
local M = SlotsHallModel
...
function M.InitUIConfig()
    ...
    this.UIConfig.slots_list = MathExtend.SortListCom(config.game, sort)
    this.UIConfig.slots_map = {}
    for k,v in ipairs(config.game) do
        this.UIConfig.slots_map[v.line] = v
    end
end

function M.GetIdByCfg(id)
    return this.UIConfig.slots_map[id]
end
function M.GetLayoutCfg()
    local _config = this.UIConfig.slots_list
    local layout_cfg = {}
    for i = 1, #_config do
        if _config[i].is_onoff == 1 then
            layout_cfg[#layout_cfg + 1] = _config[i]
        end
    end
    return layout_cfg
end
