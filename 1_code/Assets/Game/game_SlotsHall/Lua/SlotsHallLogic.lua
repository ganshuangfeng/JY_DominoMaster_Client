-- 创建时间:2022-03-16
local cur_path = "Game.game_SlotsHall.Lua."
ext_require(cur_path .. "SlotsHallModel")
ext_require(cur_path .. "SlotsHallGamePanel")
ext_require(cur_path .. "SlotsHallItemPrefab")

SlotsHallLogic = {}
local L = SlotsHallLogic
L.panelNameMap = {
    game = "game",
    hall = "hall"
}
...
function L.Init(parm)
    this = L
    dump(parm, "<color=red>SlotsHallLogic Init parm</color>")
    --初始化model
    local model = SlotsHallModel.Init()
    MakeLister()
    AddMsgListener(lister)
...
function L.Exit()
    if this then
        print("<color=green>Exit  SlotsHallLogic</color>")
        this = nil
        if cur_panel then
...
function L.change_panel(panelName)
    ...
    if not cur_panel then
        ...
        elseif panelName == L.panelNameMap.game then
            cur_panel = {name = panelName, instance = SlotsHallGamePanel.Create()}
        end
    end
end

return SlotsHallLogic