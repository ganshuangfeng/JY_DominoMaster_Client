...
EliminateFXItemBG = basefunc.class()
local M = EliminateFXItemBG

math.randomseed(tostring(os.time()):reverse():sub(1, 6))
M.random = math.random(1,2)

M.name = "EliminateFXItemBG"
...
function M:ctor(data)
    local parent = EliminateFXObjManager.GetBGContent()
    if not parent then return end
    self.data = data
    self.ui = {}
	self.ui.gameObject = GameObject.Instantiate(EliminateFXObjManager.item_obj.EliminateFXItemBG,parent)
    self.ui.transform = self.ui.gameObject.transform
    self.ui.transform.localPosition = eliminate_fx_algorithm.get_bg_pos_by_index(self.data.x,self.data.y)
