...
function C.init( _is_client , _service_cfg , _use_for )
	...
	if C.is_for_server then
		...
		C.load_config( ext_require("Game.CommonPrefab.Lua.permission_server_config") )
	end
	...
	elseif _use_for == "client" then
		C.asset_judge_module = ext_require("Game.CommonPrefab.Lua.permission_asset_judge_module_for_center_client")
	else
		C.asset_judge_module = ext_require("Game.CommonPrefab.Lua.permission_asset_judge_module_for_center_client")
	end
end
