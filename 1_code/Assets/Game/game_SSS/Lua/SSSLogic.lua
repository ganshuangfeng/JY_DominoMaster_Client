-- 创建时间:2022-02-10
local cur_path = "Game.game_SSS.Lua."
ext_require(cur_path .. "SSSModel")
ext_require(cur_path .. "SSSGamePanel")
...
function L.Init(parm)
    this = L
    dump(parm, "<color=red>SSSLogic Init parm</color>")
    --初始化model
    local model = SSSModel.Init()
    MakeLister()
    AddMsgListener(lister)
...
function L.Exit()
    if this then
        print("<color=green>Exit  SSSLogic</color>")
        this = nil
        if cur_panel then
...
function L.change_panel(panelName)
    ...
    if not cur_panel then
        ...
        elseif panelName == L.panelNameMap.game then
            cur_panel = {name = panelName, instance = SSSGamePanel.Create()}
        end
    end
end
